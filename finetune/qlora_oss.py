...
def apply_lora(
    ...
    dropout: float,
    use_unsloth: bool = False,
    use_gradient_checkpointing: Union[bool, str] = True,
) -> PeftModel:
    """应用 LoRA 适配器（统一入口）
    ...
    if use_unsloth and UNSLOTH_AVAILABLE:
        # 处理use_gradient_checkpointing参数
        if isinstance(use_gradient_checkpointing, str) and use_gradient_checkpointing.lower() == "unsloth":
            gradient_checkpointing_value = "unsloth"
        else:
            gradient_checkpointing_value = bool(use_gradient_checkpointing)
        ...
        model = FastLanguageModel.get_peft_model(
            ...
            bias="none",
            use_gradient_checkpointing=gradient_checkpointing_value,
            random_state=3407,
        )
    else:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="True",
        help="梯度检查点设置，可以是True/False或'unsloth'",
    )
    parser.add_argument(
        "--full_finetuning",
        type=lambda x: str(x).lower() == "true",
        default=False,
        help="是否使用全参数微调而非LoRA",
    )
    parser.add_argument(
        "--merge_and_save",
        action="store_true",
