#!/usr/bin/env python3
"""
简化版 QLoRA 微调脚本 - 支持 Unsloth 和普通 4bit 量化
专为稳定性和兼容性优化，支持 Windows 和 Linux
"""
...
def log_gpu_memory_usage(step_name: str):
    """记录GPU显存使用情况"""
    if torch.cuda.is_available():
        allocated = torch.cuda.memory_allocated() / 1024**3
        reserved = torch.cuda.memory_reserved() / 1024**3
        print_rank0(f"{step_name} - GPU显存使用: 已分配 {allocated:.2f} GB, 已保留 {reserved:.2f} GB")
    else:
        print_rank0(f"{step_name} - CUDA不可用")

def safe_dtype_preference():
    """选择合适的数据类型"""
...
def load_model_and_tokenizer(base_dir: str, use_qlora: bool = True, use_unsloth: bool = False, use_gradient_checkpointing: bool = True):
    """加载模型和分词器，支持 Unsloth 和普通 4bit 量化"""
    print_rank0("加载分词器...")
    ...
    print_rank0("模型已准备好进行8bit量化训练")
    
    return model, tokenizer

def apply_lora(model, target_modules: List[str], r: int, alpha: int, dropout: float):
    """应用 LoRA 适配器"""
...
def merge_and_save(base_dir: str, output_dir: str):
    """合并 LoRA 权重并保存完整模型"""
    ...
    # 加载 PEFT 模型
    peft_model = PeftModel.from_pretrained(model, output_dir, device_map="auto")
...
def parse_args():
    ...
    parser.add_argument("--use_unsloth", type=lambda x: str(x).lower() == "true", default=False, 
    ...
    parser.add_argument("--dataloader_pin_memory", type=lambda x: str(x).lower() == "true", default=False, 
...
def main():
    ...
    model, tokenizer = load_model_and_tokenizer(base_dir, use_qlora=args.use_qlora, use_unsloth=args.use_unsloth, use_gradient_checkpointing=args.gradient_checkpointing)
    ...
    training_args = TrainingArguments(
        ...
        report_to=[],
        dataloader_pin_memory=args.dataloader_pin_memory,  # 使用命令行参数控制pin_memory
        dataloader_num_workers=args.dataloader_num_workers,  # 使用命令行参数控制工作线程数
    )
