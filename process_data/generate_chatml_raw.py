...
class ChatMLGenerator:
    ...
    def __init__(self, input_path: str = None, output_path: str = None):
        ...
        # 支持自定义输入输出路径
        self.input_path = input_path or "dataset/csv"
        self.output_path = output_path or "./dataset/sft.jsonl"
    ...
    def run(self):
        ...
        try:
            ...
            # 加载CSV数据
            messages = self.load_csv_files(self.input_path)
            ...
            # 导出数据
            self.export_to_chatml_format(qa_pairs, self.output_path)
...
def main():
    """主函数"""
    import argparse
    parser = argparse.ArgumentParser(description='生成ChatML格式训练数据')
    parser.add_argument('--input', help='输入CSV目录路径', default=None)
    parser.add_argument('--output', help='输出JSONL文件路径', default=None)
    
    args = parser.parse_args()
    
    generator = ChatMLGenerator(input_path=args.input, output_path=args.output)
    generator.run()
