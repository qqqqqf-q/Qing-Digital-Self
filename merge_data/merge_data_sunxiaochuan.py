...
def convert_qa_to_new_format(qa_data: List[Dict[str, Any]], system_prompt: str) -> List[Dict[str, Any]]:
    """将qa数据转换为新的格式，支持messages格式，添加system prompt"""
    converted_data = []
    
    for qa_item in qa_data:
        # 支持三种格式：messages格式、turns数组格式、instruction/input/output格式
        if 'messages' in qa_item:
            # messages格式 - 已经是chatml格式，只需添加system prompt
            messages = qa_item['messages']
            if not messages:
                continue
                
            # 在消息列表开头添加system prompt
            ...
            converted_data.append({"messages": new_messages})
        elif 'turns' in qa_item:
            # 原始turns格式
            turns = qa_item['turns']
            if len(turns) < 2:
                continue
