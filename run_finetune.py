...
def main():
    ...
    logger.info(
        zhcn="启动微调...",
        en="Starting fine-tuning..."
    )
    logger.info(
        zhcn=f"命令: {' '.join(cmd)}",
        en=f"Command: {' '.join(cmd)}"
    )
    logger.info(
        zhcn="环境变量(关键项):",
        en="Environment variables (key items):"
    )
    ...
    try:
        ...
        logger.error(
            zhcn=f"训练失败，退出码: {e.returncode}",
            en=f"Training failed, exit code: {e.returncode}"
        )
        return e.returncode
    except KeyboardInterrupt:
        logger.warning(
            zhcn="训练被用户中断",
            en="Training interrupted by user"
        )
        return 1
